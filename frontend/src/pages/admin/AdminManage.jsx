...
export default function AdminManage() {
  ...
  const selected = useMemo(() => items.find(r => r._id === selectedId), [items, selectedId])
  ...
  return (
    <div className="admin-manage">
      ...
      <div className="filters">
        <div className="field">
          <label>Ward</label>
          <input value={filters.ward} onChange={(e) => setFilters({ ...filters, ward: e.target.value })} placeholder="e.g. 5" />
        </div>
        <div className="field">
          <label>Status</label>
          ...
          <div className="table-wrapper">
            <table className="requests-table">
              ...
              <tbody>
                ...
                {items.map(r => (
                  <tr key={r._id} className={selectedId === r._id ? 'active' : ''} onClick={() => setSelectedId(r._id)}>
                    <td>{r.ticketId}</td>
          ...
          {selected && (
            <div className="card details">
              <div className="header">
                ...
                <select className="status-select" value={selected.status} onChange={(e) => updateStatus(selected._id, e.target.value)}>
                  {['Open','Assigned','On the Way','In Progress','Completed','Rejected'].map(s => (
                    <option key={s} value={s}>{s}</option>
                  ))}
                </select>
              ...
              <div className="grid-2">
                ...
                <div>
                  ...
                  <div className="field-row">
                    ...
                    <select defaultValue={selected.assignedOperator?._id || ''} onChange={(e) => assignOperator(selected._id, e.target.value)}>
                ...
                <div className="section">
                  ...
                  <ul className="notes">
                    {selected.notes.map((n, i) => (
                      <li key={i}><span className="muted">[{new Date(n.createdAt).toLocaleString()}] {n.role}:</span> {n.text}</li>
                    ))}
                  </ul>
                </div>
              )}
